...
## II Training
This section will introduce how to train a detection model of YOLO with your own data set.
...
```
- To achieve this, we could use [labelImg](https://github.com/tzutalin/labelImg) to annotate raw images and directly generate TXT format annotations. If you already have other format annotations, you may use a python script or this project [Annotation Coverting](https://github.com/ssaru/convert2Yolo) to transform them to TXT format.
...
- change [`filters=36`] to **filters=(classes + 5)x3** in the 3 `[convolutional]` before each `[yolo]` layer, keep in mind that it only has to be the last `[convolutional]` before each of the `[yolo]` layers. So if `classes=2` then write `filters=21`, if `classes=7` then write `filters=36`.
###### **obj.names**
...
- For more details about training YOLO, you could see the original document of darknet: [How to train YOLO with your own dataset](https://github.com/AlexeyAB/darknet#how-to-train-to-detect-your-custom-objects)
- **Reference**:
...
3. https://github.com/ssaru/convert2Yolo
